...
- `virtual.target_total` : 목표 대기열 인원 (기본 100)
- `virtual.initial_jump_ratio` : 시작 시 즉시 주입 비율 (기본 0.3)
- `virtual.burst_duration_sec` : 목표 인원까지 도달하는 시간 (기본 30)
- `virtual.inject_batch_size` : 배치 주입 크기 (기본 10)
- `virtual.inject_batch_delay_ms` : 배치 간 대기 시간 (기본 1000)
- `virtual.enabled` : 가상 유저 기능 활성화 (기본 false)

#### worker

- `worker.transfer_interval_sec` : 대기열 스케줄링 간격 (기본 3)
- `worker.max_capacity` : 활성 큐 최대 수용 인원 (기본 10)
- `worker.heartbeat_timeout_ms` : 하트비트 미수신 허용 시간 (기본 60000)
- `worker.active_ttl_ms` : 활성 유저 상태 TTL (기본 60000)
...
- `virtual.process_delay_ms` : 예약 처리 후 대기 시간 (기본 1000)
...
```redis
HSET config:queue worker.transfer_interval_sec 10
